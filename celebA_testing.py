...
def train_reg(model, criterion, optimizer, trainloader, testloader, n_epochs, device='cpu'):
    ...
    for epoch in range(n_epochs):
        ...
        test_acc = test(model, criterion, testloader,device)  # Implement your test function accordingly
        ...
        torch.save(checkpoint, 'best_model_checkpoint_Prelu_152_IR_224_overfit.pth')
...
NUM_CLASSES = 2
BATCH_SIZE = 128
DEVICE = DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'  # Use CUDA device if availablecpu' # default GPU device
GRAYSCALE = False
...
##### Creating an optional reduced subset##############
# from torch.utils.data import Subset
# import numpy as np
# num_samples_to_keep = len(celeba_dataset) // 100

# # Generate a random subset of indices to keep
# subset_indices = np.random.choice(len(celeba_dataset), num_samples_to_keep, replace=False)

# # Create a new dataset from the subset of indices
# reduced_celeba_dataset = Subset(celeba_dataset, subset_indices)

# celeba_dataset = reduced_celeba_dataset
...
optimizer = torch.optim.SGD(params=model.parameters(),
             					...
             					momentum= 0.9, 
             					weight_decay=1e-4)
model.load_pretrained_weights(checkpoint_path='best_model_checkpoint_Prelu_152_IR_224_new.pth')
...
checkpoint = torch.load('best_model_checkpoint_Prelu_152_IR_224_new.pth')

# Extract the trainloader from the checkpoint
train_loader = checkpoint['trainloader']
acc = test(model, criterion= nn.CrossEntropyLoss(), dataloader=train_loader, device='cpu')
...
#         }
#torch.save(checkpoint, 'best_model_checkpoint_Prelu_152_IR_224_new.pth')

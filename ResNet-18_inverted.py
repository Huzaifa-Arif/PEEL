...
def basic_block_inverted(res_block_out,res_block_in,conv1_params:dict,conv2_params:dict,conv3_params:dict,shortcut = None):
    ...
    for layer_name in reversed(list(resnet18_info.keys())):
        ...
        for block_name in reversed(list(layer_info.keys())):
           ...
           # import pdb;pdb.set_trace()
            ...
            plot_channels(res_block_in,rec = True)
            plot_channels(inverted_input,rec = False)
            
            

           # print("original_block_out shape:", orignal_block_out.shape)
           # print("inverted_input shape:", inverted_input.shape)

            
            #import pdb;pdb.set_trace()
            error = (torch.norm(res_block_in - inverted_input) / (res_block_in.view(-1).shape[0]))* 100
            print(f"The error per layer {layer_name} and block {block_name} is {error.item()}%")
...
def main():
    ...
    resnet= ResNet18(pretrained=False,resblock_length=18) # 18,34,50,152
    ...
    if(resblocks):
        ...
        inverted_input,orignal_input = resblock_inverted(resnet_info,resnet)
    ...
    if(first_layer):
        ...
        if(resblocks):
            ...
            first_layer = resnet.activations['after_conv1'] 
        ...
        # Create the new directory in the home directory
        home_dir = os.path.expanduser('~')
        new_dir_path = os.path.join(home_dir, new_dir)
        ...
        # Define the filename
        filename = 'resblock-152.png'
        
        # Construct the full path for saving the image
        save_path = os.path.join(new_dir_path, filename)
        ...
        # Now call the plot_image function with the new save_path
        plot_image(input_recovered, save_path=save_path)
        
        plot_image(input_image)
    
        
    error = (torch.norm(input_image - input_recovered) / (input_image.view(-1).shape[0])) * 100
    print(f"The error per image is {error.item()}%")

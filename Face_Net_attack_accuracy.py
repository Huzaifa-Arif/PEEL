...
class Recovered_ImageDataset(Dataset):
    def __init__(self, main_dir, transform, file_list=None):
        ...
        if file_list is None:
            # Filter to include only PNG images
            self.all_imgs = sorted([f for f in os.listdir(main_dir) if f.endswith('.png')])
        else:
    ...
    def __getitem__(self, idx):
        if isinstance(self.all_imgs[idx], str):
            ...
            raise TypeError(f"Expected string, got {type(self.all_imgs[idx])} at index {idx}")
...
def main():
    ...
    checkpoint = torch.load('best_model_checkpoint_faceevolve_224_new.pth',map_location=torch.device('cpu') )
    train_loader = checkpoint['loader']
    ...
    if(diff):
      eval_model = IR152()
      eval_checkpoint = torch.load('best_model_checkpoint_Prelu_152_IR_224_new.pth',map_location=torch.device('cpu') )
   ...
   # proc.append(transforms.ToTensor())
    ...
    reconstructed_file_name = 'recovered_images_celebA_FaceNet64_small' #'recovered_images_celebA_FaceNet64' # 'recovered_images_celebA_IR152'
    input_file_name = 'input_images_celebA_FaceNet64_small'#'input_images_celebA_FaceNet64' #'input_images_celebA_IR152'
    
    recovered_images_files = get_sorted_file_list(reconstructed_file_name)
    input_images_files = get_sorted_file_list(input_file_name)
    ...
    dataset = Recovered_ImageDataset( input_file_name, transform=transform, file_list=input_images_files)

...
def find_shortest_dist(fea_target,fea_fake):
    ...
    fea_target = torch.from_numpy(fea_target)
    fea_fake = torch.from_numpy(fea_fake)
...
#### Get a dictionary such that the dataloader stores each label has a tensor for each image
